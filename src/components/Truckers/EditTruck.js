  ...
  const handleSubmit = async (e) => {
    ...
    if (response.data.code === 1) {
      if (!(picture === null)) {
        ...
        const imageResponse = await authService.uploadImage(formData1, id)
      }
      if (!(backLisencePicture === null && lisencePicture === null)) {
        ...
        const LicenseimageResponse = await driverService.UploadDrivingLicense(id, formData2)
      }

      if (!(insurencePicture === null)) {
        const formData4 = new FormData()
        formData4.append("file", insurencePicture)
        const InsurenseResponse = await driverService.UploadInsurece(id, insurenceInfo, formData4)
      } else if (!(insurenceInfo === "" || insurencePicture === null)) {
        const formData4 = new FormData()
        formData4.append("file", insurenceImgData)
        const InsurenseResponse1 = await driverService.UploadInsurece(id, insurenceInfo, formData4)
