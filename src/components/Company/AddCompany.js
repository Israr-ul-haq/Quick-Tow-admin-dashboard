...
function AddCompany() {
    ...
    const handleSubmit = async (e) => {
        ...
        debugger
        const response = await companyService.saveCompany(user)
        if (response.data.code === 1) {
            const formData = new FormData()
            formData.append("file", picture)
            const imageResponse = await companyService.uploadImage(formData, response.data.data.user.companies[0].userId)
                            ...
                            <div className="col-12">
                                <div className="headertopbar">
                                    <Link to="/ManageCompany" className="headertopbar_title">
                                        {" "}
                                        <img className="headertopbar__arrowimage" alt="back arrow" src="./img/Icon ionic-ios-arrow-back.svg" /> Add Company
                                    </Link>
                                </div>
                            </div>
                        ...
                        <div className="form-group col-md-12 formbtncontainer__outercontainer--layout3">
                            <div className="formbtncontainer">
                                <button type="submit" disabled={btnLock} className="profile-business-accept btn btn-primary">
                                    Save {btnLock ? <div className="btnloader">{Loader}</div> : ""}
                                </button>
